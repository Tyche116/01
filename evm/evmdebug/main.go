...
import (
	...
	"github.com/tendermint/tendermint/evm"
	"github.com/tendermint/tendermint/evm/abi"
...
func main() {
	...
	// return
	// 创建账户State
	stateDb, err := evm.TryLoadFromDisk()
	if err != nil {
		panic(err)
	}

	blockContext := evm.NewEVMBlockContext(normalAccount, 100, 1200000, 1, 0xfffffffffffffff)
	vmenv := vm.NewEVM(blockContext, evm.TxContext{}, stateDb, vm.Config{})
